...
export default function SectionRail() {
    ...
    // Rail is hidden at rest and reveals only while the user is actively
    // scrolling; it fades back out a beat after scrolling stops.
    const [isScrolling, setIsScrolling] = useState(false);
    const shouldReduceMotion = useReducedMotion();

    useEffect(() => {
        // Reduced motion: keep the rail permanently visible — no fade dance.
    ...
    return (
        <nav
            ...
            className={`hidden lg:flex fixed left-8 top-1/2 -translate-y-1/2 z-40 flex-col transition-opacity duration-500 ${
